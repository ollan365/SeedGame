...
public class BackendPostSystem : MonoBehaviour
{
				...
				for (int i = 0; i < jsonData.Count; ++i)
				{
					...
					foreach (LitJson.JsonData itemJson in jsonData[i]["items"])
					{
						...
						if (itemJson["chartName"].ToString() == Constants.GOODS_CHART_NAME)
						{
							...
							if (post.postReward.Equals(itemName))
							{
	...
	public void PostReceive(PostType postType, int index)
	{
		...
		Debug.Log($"{postType}의 {postList[index].inDate} 우편수령을 요청합니다.");

		Backend.UPost.ReceivePostItem(postType, postList[index].inDate, callback =>
		{
			if (!callback.IsSuccess())
			{
				Debug.LogError($"{postType}의 {postList[index].inDate} 우편수령 중 에러가 발생했습니다. : {callback}");
				return;
			}

			Debug.Log($"{postType}의 {postList[index].inDate} 우편수령에 성공했습니다. : {callback}");
		...
		try
		{
			foreach (LitJson.JsonData itemJson in item)
			{
				...
				if (chartName.Equals(Constants.GOODS_CHART_NAME))
				{
					...
					{
						BackendGameData.Instance.UserGameData.seed += itemCount;
						TM.UpdateGameData();
					}
					else if (itemName.Equals("goldSeed"))
					{
						BackendGameData.Instance.UserGameData.goldSeed += itemCount;
						TM.UpdateGameData();
					}
                    else if(itemId >= 2 && itemId <= 7)
                    {
						BackendGameData.Instance.inventory.ChangeSeedNum(itemId - 2, itemCount);
                    }
					else if(itemId >= 8 && itemId <= 13)
                    {
						BackendGameData.Instance.inventory.ChangeFruitNum(itemId - 8, itemCount);
					}
				}
